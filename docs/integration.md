...
- 논문은 **연도별로** 관리한다. 각 항목에 `year` 와 `question`(q1~q4)이 붙고,
  ...
  | Semantic Scholar | 저널 / SSRN·arXiv 는 프리프린트 | externalIds 로 SSRN 워킹페이퍼를 잡는다 |
...
## 7-d. 에이전트 대화를 화면으로 되돌리기

자동 실행은 `export-brief.js` 로 **화면 없이** 돈다. 리포트만 남으므로, 나중에
화면을 열면 "실행만 되고 대화를 안 한 것"처럼 보인다. **대화는 리포트 안에
그대로 있다** — 재생 경로가 그것을 화면으로 되돌린다.

```
export-brief.js --run   →  reports/*.md  (대화가 이 안에 있다)
                            ↓
GET /api/replay?file=latest  →  replay.js 가 마크다운을 SSE 이벤트로 복원
                            ↓
화면이 실제 런과 똑같이 흘러간다 (실제 분석은 다시 돌지 않는다)
```

진입점은 셋이고 전부 같은 곳으로 간다.

| | |
|---|---|
| `RUN_ALL.cmd` 6/6 단계 | 분석이 끝나면 화면을 새 창에 띄우고 `?replay=latest` 로 연다 |
| `PIXEL_FLOOR.cmd replay` | 화면만 켜고 최근 대화를 재생 |
| 화면의 `⏵ 리플레이` | 리포트를 골라 재생 · 배속 선택 |

`file=latest` 는 서버가 `reports/*.md` 중 mtime 이 가장 최근인 것으로 해석한다.
없으면 404 와 함께 "분석을 한 번 돌린 뒤에" 라고 답한다 — 지어내지 않는다.

`auto` 모드(스케줄러)에서는 화면을 띄우지 않는다. 아무도 안 보는 창을 남기지
않기 위해서다. `docs/audit.py` 의 `[3-c]` 가 이 사슬 다섯 마디를 검사한다 —
하나가 끊겨도 오류는 안 뜨고 "대화를 안 하는 것"으로만 보이기 때문이다.
