...
// ---------------------------------------------------------------------------
const AGENTS = [
  // 팩터를 스스로 계산하지 않는다 — 원장이 계산한 값과 그 논문을 받아, 이 종목의
  // 회수 판단에서 무엇을 어떻게 볼지 **다른 에이전트에게 제안**한다. 그래서
  // 애널리스트보다 먼저 돈다(엔진의 방법론 데스크 단계).
  { id: 'quant', name: 'QUANT', nameKo: '퀀트', role: '방법론 제안', roomKo: '퀀트 데스크', requiresKi: true },
...
// 퀀트 데스크의 제안을 받는 쪽. 전원에게 주면 열여섯이 같은 제안을 되풀이하고,
// 그 되풀이가 합의처럼 읽힌다. 팩터가 실제로 판단을 바꾸는 자리에만 준다.
const QUANT_READERS = new Set(['flow', 'diana', 'red', 'ace', 'pm']);
...
// 재료라, 전원에게 주면 17명이 같은 거시 서사를 반복하고 앙상블이 무너진다.
...
function buildPrompt(id, context = {}) {
  ...
  // 논문 팩터는 퀀트 데스크에만 간다. 전원에게 주면 열여섯이 같은 팩터 이야기를
  // 반복하고, 그 반복이 합의처럼 읽힌다.
  const kiFactorLines =
    id === 'quant' && market.kiFactors
      ? (() => {
          try {
            return kiBridge.formatKiFactorLines(market.kiFactors);
          } catch (_) {
            return [];
          }
        })()
  ...
  if (id === 'taro') {
    ...
    // 퀀트 데스크 — 논문을 읽고, 이 종목에 어떻게 쓸지 데스크에 제안한다.
    //
    // 이 자리의 실패 방식은 하나다. 논문이 말한 적 없는 것을 말했다고 읽고,
    // 그것을 숫자의 권위로 데스크에 넘기는 것. 그래서 프롬프트가 값·주장·유보를
    // 따로 주고, 출력에서도 셋을 붙여 두라고 시킨다.
    parts.push('[원장이 계산한 논문 팩터 — 값 · 근거 · 그 논문이 주장하지 않는 것]');
    parts.push(kiFactorLines.length ? kiFactorLines.join('\n') : '데이터 없음');
    parts.push('');
    // 원장의 절(§실행 시뮬레이션·§매물대·§자본구조…)은 각각 담당이 따로 있다.
    // 여기서 같이 받으면 분업이 무너지고 같은 절을 둘이 말하게 된다. QUANT 가
    // 보는 것은 팩터뿐이다.
    kiUsed = true;
    parts.push(
      '너는 퀀트 데스크(QUANT)다. **판정을 내리는 자리가 아니다.** 위 팩터를 읽고, ' +
        '이 회수 판단에서 그것을 어떻게 쓸지 **나머지 에이전트에게 제안**하는 것이 네 일이다.\n' +
        '\n' +
        '이 데스크가 답해야 하는 질문은 넷이다 — 얼마나 왔는가 / 팔 수 있는가 / ' +
        '어떻게 팔 것인가 / 지금이 그 때인가. 팩터를 이 넷 중 어디에 붙일지 정하라.\n' +
        '\n' +
        '다음 형식으로 답하라.\n' +
        '1) 이 종목에서 **가장 무겁게 봐야 할 팩터 2~3개**와 그 이유. ' +
        '값이 크다/작다가 아니라, 이 판단의 어느 질문을 바꾸는지로 말하라.\n' +
        '2) 각각에 대해 세 줄로: [값] / [논문이 실제로 주장한 것] / ' +
        '[이 종목에 적용할 때의 유보]. 유보를 생략하지 마라.\n' +
        '3) **무시해도 되는 팩터**와 그 이유. 이쪽이 더 유용할 때가 많다.\n' +
        '4) 다른 에이전트에게 주는 구체적 제안 — 누가(FLOW·DIANA·ACE·PM 중) ' +
        '무엇을 확인해야 하는지.\n' +
        '\n' +
        '지켜야 할 것.\n' +
        '· 팩터를 네가 다시 계산하지 마라. 위에 없는 숫자는 없는 것이다.\n' +
        '· 논문을 네 기억에서 새로 꺼내 인용하지 마라. 위에 붙은 인용만 쓴다. ' +
        '필요한 논문이 없으면 "그 근거는 이 자리에 없다"고 적어라.\n' +
        '· 미국 표본의 결과를 한국 코스닥 소형주에 그대로 옮기지 마라. ' +
        '옮길 수 있다고 보면 왜 그런지를 적어라.\n' +
        '· 매수·매도·목표가를 말하지 마라. 그건 ACE 와 회의의 일이다.'
    );
  } else if (id === 'filing') {
    ...
    parts.push(NUMERIC_LEVEL_RULE);
    parts.push(minRRRule(attack));
    // ACE 는 여기서 바로 돌아가므로 공통 푸터를 못 받는다. 퀀트 제안도 같다 —
    // 여기 따로 붙이지 않으면 정작 판정을 내리는 자리만 못 본다.
    if (context.quantReport && QUANT_READERS.has(id)) {
      parts.push('');
      parts.push('[퀀트 데스크 제안 — 방법론 · 판정 아님]');
      parts.push(String(context.quantReport).slice(0, 2500));
      parts.push(
        '위는 제안이지 결론이 아니다. 동의하지 않으면 그렇게 적어라.'
      );
    }
    parts.push('');
  ...
  // 퀀트 데스크의 제안. **제안이지 판정이 아니다** — 그 구분을 프롬프트에
  // 적어 두지 않으면, 받는 쪽은 앞 단계의 문장을 확정된 사실로 읽는다.
  if (context.quantReport && QUANT_READERS.has(id)) {
    parts.push('');
    parts.push('[퀀트 데스크 제안 — 방법론 · 판정 아님]');
    parts.push(String(context.quantReport).slice(0, 2500));
    parts.push(
      '위는 논문 팩터를 이 판단에 어떻게 쓸지에 대한 **제안**이다. 결론이 아니다. ' +
        '동의하지 않으면 그렇게 적어라 — 네 판단을 여기에 맞추지 마라.'
    );
  }
...
// 에이전트 모델 — 역할에 따라 나눈다.
//
// 17명이 전부 최상위 모델을 쓸 이유가 없다. 재료를 읽어 정리하는 일과,
...
function mockResult(id, context = {}) {
  ...
  const table = {
    quant: {
      bubble: `Amihud 비유동성이 이 종목에서 가장 무겁습니다. 팔 때 얼마나 밀리는지가 이번 판단의 핵심입니다.`,
      report: `이번 회수 판단에서 가장 무겁게 봐야 할 것은 유동성 계열입니다. 첫째, Amihud (2002) 비유동성은 거래대금 대비 가격이 얼마나 움직이는지를 잽니다. 논문이 주장한 것은 "비유동적일수록 요구수익률이 높다"까지이고, 이 값이 특정 가격에서 팔린다는 뜻은 아닙니다. 미국 표본이므로 코스닥 소형주의 수준값을 그대로 옮길 수 없고, 같은 종목의 시계열 변화로 읽는 편이 안전합니다. 둘째, Corwin·Schultz (2012) 고저가 스프레드는 호가 데이터 없이 거래비용의 대리값을 줍니다. 실시간 호가가 막혀 있어도 쓸 수 있다는 점이 이 자리에서 중요합니다. 다만 장 시작 갭을 완전히 걷어내지 못하므로 절대수준보다 추세로 보십시오. 셋째, 상장 후 경과는 Field·Hanka (2001) 의 락업 만료 효과와 맞물립니다. 다만 경과 개월은 상장일에서 센 달력값일 뿐 실제 의무보유확약 기간이 아니므로, 원문은 증권신고서로 확인해야 합니다. 반대로 무시해도 되는 것은 12-1 모멘텀입니다 — 이미 보유 중인 자산의 처분 시점 문제이지 진입 신호가 아니고, 표본도 다릅니다. 제안드립니다. FLOW 는 Amihud 와 실행 시뮬레이션의 소요일수가 같은 방향을 가리키는지 대조해 주십시오. DIANA 는 상장 후 경과와 실적 궤적이 어긋나는지 봐 주십시오. ACE 는 스프레드 추정치를 목표단가에서 차감한 뒤에도 회수 기준을 넘는지 확인해 주십시오. PM 은 락업 관련 물량이 다른 보유 종목과 같은 시기에 겹치는지 보셔야 합니다.`,
    },
    taro: {
