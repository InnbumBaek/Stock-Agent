...
공공기관 API 로 잰 값과 언론사 제목을 같은 무게로 프롬프트에 넣으면, 리포트에서
"뉴스에서 봤다"가 "실측했다"처럼 읽힌다. 회의에서 그 구분이 사라지는 것이 이
시스템의 가장 조용한 실패다.

**1차** — KRX · DART · 한국은행 ECOS · KIS. 근거로 삼아도 된다.
**참고** — 구글 뉴스 헤드라인 · 공포탐욕지수. "이런 이야기가 돌고 있다"까지만.

`agents.js` 의 `SOURCE_RULE` 이 16명 전원에게 붙고 `test/roster.test.mjs` 가
강제한다. 새 재료를 붙일 때는 **어느 등급인지 먼저 정하고**, 참고 등급이면
블록 제목에 그 사실을 적어라.

**에이전트가 자기 기억에서 숫자를 꺼내 쓰는 것도 금지다.** 학습 시점에 멈춘
값이고, 틀렸을 때 아무도 추적할 수 없다.

### 6. 에이전트 출력은 신뢰할 수 없는 입력이다

에이전트 리포트는 언어모델이 만든 문자열이고, 그것이 HTML 리포트에 들어간다.
반드시 `_esc()` 로 이스케이프해라. `selftest` 가 `<script>` 주입을 검사한다.

### 7. 대외비·자격증명을 저장소에 넣지 마라
...
```bash
cd stock-monitor && python ki_monitor.py selftest   # 117개 (키·네트워크 불필요)
