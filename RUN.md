...
RUN_ALL.cmd demo     에이전트를 가짜 응답으로 — 무료
RUN_ALL.cmd noai     에이전트를 건너뛰고 숫자 리포트만
RUN_ALL.cmd auto     묻지 않고 끝까지 — 스케줄러용
```

환경 점검 → API 진단 → 원장 → 설정 → 에이전트 → 리포트를 순서대로 돌리고,
...
- **비싼 단계는 물어봅니다.** 에이전트 실전 런은 종목당 opus 최대 16콜입니다.
  `demo`·`noai` 로 건너뛸 수 있고, `claude` CLI 가 없으면 자동으로 데모가 됩니다.

> 이건 **수동 실행기**입니다. 매주 자동으로 도는 일정(금요일 분석 → 월요일 리포트)은
> `INSTALL_SCHEDULE.cmd` 가 따로 등록하며, `RUN_ALL` 과 섞이지 않습니다.
...
## 6-4. 실시간 시세가 제대로 붙었는지 (KIS 키를 넣으셨다면)
...
`"ok": true` 와 `"price"` 가 나오면 붙은 것입니다. 그 뒤로 리포트와 에이전트
프롬프트의 시세 줄이 이렇게 바뀝니다.
...
| 나오는 것 | 뜻 |
|---|---|
| `"ok": false` · `KIS_APP_KEY 가 .env 에 없습니다` | 키를 안 넣으신 겁니다. **안 넣어도 됩니다** — 원장 종가로 돕니다 |
| `"ok": false` · `EGW00133` | 토큰 재발급 제한입니다. 잠시 뒤 다시 하십시오 |
| `"market_open": false` | 정규장 밖입니다. 정상이고, 원장 종가로 돌아갑니다 |
| `검산 실패` | 응답 필드가 예상과 다릅니다. **이때는 값을 내보내지 않습니다** — 틀린 현재가는 없는 것보다 나쁘기 때문입니다 |

> 주문 API 는 화이트리스트에 없어 호출되지 않습니다. 조회 두 개뿐입니다.

---

## 6-5. 장중 알림 (선택)

포지션을 넣어 두면 장중에 폴링하며 규칙에 걸릴 때만 알립니다.

```bash
python ki_monitor.py init          # positions.csv 뼈대를 만듭니다
python ki_monitor.py watch --once  # 1회만 순회 (연결 시험용)
python ki_monitor.py watch         # 장중 상시
```

`--once` 로 먼저 한 바퀴 돌려 보십시오. 상시 실행은 KIS 호출이 계속 나갑니다.
